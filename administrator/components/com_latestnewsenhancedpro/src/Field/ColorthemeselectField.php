...
class ColorthemeselectField extends DynamicsingleselectField
{
	...
	protected function getOptions()
	{
		...
		foreach ($optionsArray as $option) {
			...
			$lang->load('com_latestnewsenhancedpro_style_colortheme_'.$option);

			$translated_option = Text::_('COM_LATESTNEWSENHANCEDPRO_STYLE_COLORTHEME_'.$upper_option.'_LABEL');

			$description = '';
			if (empty($translated_option) || substr_count($translated_option, 'LATESTNEWSENHANCEDPRO') > 0) {
				$translated_option = ucfirst($option);
			} else {
				$description = Text::_('COM_LATESTNEWSENHANCEDPRO_STYLE_COLORTHEME_'.$upper_option.'_DESC');
