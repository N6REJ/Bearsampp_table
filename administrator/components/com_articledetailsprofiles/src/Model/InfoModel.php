...
namespace SYW\Component\ArticleDetailsProfiles\Administrator\Model;
...
class InfoModel extends BaseDatabaseModel
{
	...
	protected $server_remote_uri = 'https://updates.simplifyyourweb.com/pro/articledetailsprofiles/';
	...
	public function getUpdateInformation()
	{
	    ...
	    try {
	        $response = $http->get($this->server_remote_uri . 'articledetailsprofiles-j4.json');
	...
	public function getLicenseState(Http $httpClient)
	{
	    ...
	    if (!PluginHelper::isEnabled('installer', 'adpinstaller')) { // the plugin does not exist
	        return array('download_id' => '');
	    }

	    $installer_plugin = PluginHelper::getPlugin('installer', 'adpinstaller');
	...
	public function getExtendedPlugins()
	{
		...
		$query = $db->getQuery(true)
			...
			->where('folder = ' . $db->quote('articledetails'))
			->where('state IN (0,1)')
			->order('ordering');
