...
class ImageServiceProviderLaravelRecent extends ServiceProvider
{
    ...
    {
        return \class_exists('RegularLabs\\Scoped\\Intervention\\Image\\ImageCache');
    }
    /**
    ...
    protected function bootstrapImageCache()
    {
        ...
        if (\is_string(config('imagecache.route'))) {
            ...
            $app['router']->get(config('imagecache.route') . '/{template}/{filename}', ['uses' => 'RegularLabs\Scoped\\Intervention\\Image\\ImageCacheController@getResponse', 'as' => 'imagecache'])->where(['filename' => $filename_pattern]);
        }
    }
    /**
