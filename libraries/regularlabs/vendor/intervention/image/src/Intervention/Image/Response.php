...
use RegularLabs\Scoped\Illuminate\Support\Facades\Response as IlluminateResponse;
use RegularLabs\Scoped\Symfony\Component\HttpFoundation\Response as SymfonyResponse;
class Response
{
    /**
    ...
    public function make()
    {
        ...
        $length = \strlen($data);
        if (\function_exists('RegularLabs\\Scoped\\app') && \is_a($app = app(), 'RegularLabs\\Scoped\\Illuminate\\Foundation\\Application')) {
            $response = IlluminateResponse::make($data);
            $response->header('Content-Type', $mime);
            $response->header('Content-Length', $length);
        } elseif (\class_exists('RegularLabs\\Scoped\\Symfony\\Component\\HttpFoundation\\Response')) {
