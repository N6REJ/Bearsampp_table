...
class Version
{
    ...
    {
        return \trim(\RegularLabs\Library\Extension::getXmlValue('version', $alias, $type, $folder));
    }
    /**
    ...
    private static function getMessageText(string $alias, string $name, string $version) : string
    {
        ...
        if (!$is_pro) {
            $msg .= ' <a href="https://regularlabs.com/purchase/cart/add/' . $alias . '" target="_blank" class="btn btn-large btn-primary">' . '<span class="icon-basket"></span> ' . JText::_('RL_GO_PRO') . '</a>';
        }
