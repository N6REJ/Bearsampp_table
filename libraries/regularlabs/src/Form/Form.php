...
class Form
{
    ...
    public static function selectListAjax(string $field_class, string $name, mixed $value, string $id, array $attributes = [], bool $treeselect = \false, bool $collapse_children = \false) : string
    {
        ...
        return '<div>' . '<textarea name="' . $name . '" id="' . $id . '" cols="40" rows="5" class="w-100"' . ' data-rl-ajax="' . \htmlspecialchars(\json_encode($ajax_data)) . '">' . $value . '</textarea>' . '<div class="rl-spinner"></div>' . '</div>';
    }
}
