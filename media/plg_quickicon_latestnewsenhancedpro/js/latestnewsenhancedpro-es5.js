...
(function (document) {
  ...
  var fetchUpdateLNEP = function fetchUpdateLNEP() {
    if (Joomla.getOptions('js-latest-news-enhanced-check')) {
      var options = Joomla.getOptions('js-latest-news-enhanced-check');
      var update = function update(type, text, linkHref) {
        var link = document.getElementById('plg_quickicon_latestnewsenhancedpro');
        ...
        if (Array.isArray(request)) {
          if (request.length === 0) {
            ...
            if (updateInfo.latest !== options.version) {
              update('danger', Joomla.Text._('PLG_QUICKICON_LATESTNEWSENHANCEDEXTENDED_VERSION_OUTDATED').replace('%s', "<span class=\"badge text-dark bg-light\"> \u200E ".concat(updateInfo.latest, "</span>")), options.updateUrl);
            } else {
