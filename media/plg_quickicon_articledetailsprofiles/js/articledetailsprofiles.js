...
(document => {
	...
	const fetchUpdateADP = () => {
		if (Joomla.getOptions('js-article-details-profiles-check')) {
		    const options = Joomla.getOptions('js-article-details-profiles-check');
		    const update = (type, text, linkHref) => {
			    const link = document.getElementById('plg_quickicon_articledetailsprofiles');
		      ...
		      if (Array.isArray(request)) {
		        if (request.length === 0) {
		          ...
		          if (updateInfo.latest !== options.version) {
		            update('danger', Joomla.Text._('PLG_QUICKICON_ARTICLEDETAILSPROFILES_VERSION_OUTDATED').replace('%s', `<span class="badge text-dark bg-light"> \u200E ${updateInfo.latest}</span>`), options.updateUrl);
		          } else {
