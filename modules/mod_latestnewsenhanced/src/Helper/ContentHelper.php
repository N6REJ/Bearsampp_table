...
class ContentHelper
{
	...
	static function getList($params, $module)
	{
		...
		if (isset($array_of_category_values['all']) && $array_of_category_values['all'] > 0) { // 'all' was selected therefore no filtering
			...
			if (!empty($categories_array)) {
				...
				if (Folder::exists(JPATH_ADMINISTRATOR . '/components/com_multicats') && ComponentHelper::isEnabled('com_multicats')) {
				    ...
				    $query->where('CONCAT(",", ' . $db->quoteName('mc.catid') . ', ",") REGEXP ",(' . implode('|', $categories_array) . '),"');
		...
		$image_types = array('image', 'imageintro', 'imagefull', 'author', 'allimagesasc', 'allimagesdesc', 'categoryimage');
		...
		foreach ($items as $key => &$item) {
			...
			if (substr($title_type, 0, strlen('jfield:text')) === 'jfield:text') {

				$type_temp = explode(':', $title_type); // $title_type can be jfield:text:fieldid
			...
			if ($cat_link_type == 'view' && $cat_view) {
				if ($cat_view == 'auto') {
					$link_temp = 'index.php?option=com_latestnewsenhancedpro&view=articles&category=' . $item->catid;
				} else {
					$link_temp = 'index.php?option=com_latestnewsenhancedpro&view=' . $cat_view_name . '&Itemid=' . $cat_view_id . '&category=' . $item->catid;
				}
			} else {
				$link_temp = RouteHelper::getCategoryRoute($item->cat_slug, $item->language); // build even if 'none' is selected for B/C
			}
			...
			} else if ($link_to == 'urla' || $link_to == 'urlb' || $link_to == 'urlc') { // links a, b and c
			...
			if ($item->state == 1) { // for published articles only
			...
			if (substr($head_type, 0, strlen('jfield:sywicon')) === 'jfield:sywicon') {
				$type_temp = explode(':', $head_type); // $head_type can be jfield:sywicon:fieldid
			...
			if (Helper::isHeadVideo($head_type) !== false) {
			    ...
			    $type_temp = explode(':', $head_type); // $head_type can be jfield:acfyoutube:fieldid
			    ...
			    if (empty($item->video)) {
			        ...
			        if ($params->get('default_videopic', '') != '') {
			        	$item->imagetag = HTMLHelper::_('image', $params->get('default_videopic', ''), $item->title, array('width' => $head_width, 'height' => $head_height));
			        }
			    } else {
			        // there is a video
...
// 						}
						...
						$type_temp = explode(':', $head_type); // $head_type can be jfield:media:fieldid
						...
						$type_temp = explode(':', $head_type); // $head_type can be jfieldusers:media:fieldid
			...
			} else if (substr($head_type, 0, strlen('jfield:calendar')) === 'jfield:calendar') {

				$item->calendar_date = null;

				$type_temp = explode(':', $head_type); // $head_type can be jfield:calendar:fieldid
			...
			if ($link_to == 'item' && $link_target == 4) {
				...
				//$item->inlinetext = JHTML::_('content.prepare', $item->inlinetext);
				PluginHelper::importPlugin('content');
				Factory::getApplication()->triggerEvent('onContentPrepare', array('com_content.article', &$item, &$item->params, 0)); // works on item->text

				$item->inlinetext = $item->text;
			}
			...
			if (!$item->authorized && !$show_unauthorized_text) {
				...
				if (substr($text_type, 0, strlen('jfield:textarea')) === 'jfield:textarea'
		            ...
		            $type_temp = explode(':', $text_type); // $text_type can be jfield:textarea:fieldid
