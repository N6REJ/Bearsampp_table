...
$layout = $params->get('layout', '_:default'); // returns _:default or <template>:default or <datasource>:_:default or <datasource>:<template>:default ...

if ($datasource != 'articles' && $datasource != 'k2') {

    // remove datasource from layout
    $layout_elements = explode(':', $layout);
    unset($layout_elements[0]);
    $layout = implode(":", $layout_elements);
}
...
switch ($datasource)
{
    ...
    default: // plugins
        ...
        $results = Factory::getApplication()->triggerEvent('onLatestNewsEnhancedGetData', array($datasource, $params, $module));
...
if (empty($list)) { // $list can be an empty array
    ...
    if (!empty($nodata_message)) {
        if ($datasource == 'articles' || $datasource == 'k2') {
            require ModuleHelper::getLayoutPath('mod_latestnewsenhanced', $layout);
        } else {
            require PluginHelper::getLayoutPath('latestnewsenhanced', $datasource, $layout);
        }
    } else {
        return;
    }
    ...
    $load_fontawesome = ($params->get('load_fontawesome', '') == '') ? $config_params->get('load_fontawesome', 0) : $params->get('load_fontawesome', '');
    if ($load_fontawesome) {
    ...
    $info_block_placement = $params->get('ad_place', 1); // keep for backward compatibility
    ...
    $load_remotely = ($params->get('remote_libraries', '') == '') ? $config_params->get('remote_libraries', 0) : $params->get('remote_libraries', '');

    // link

    $link_label = trim($params->get('link', ''));
    $unauthorized_link_label = trim($params->get('unauthorized_link', ''));
        ...
        foreach ($pos_category as $choice) {
            switch ($choice)
            {
                case 'first':
                    ...
                    break;
                case 'picture':
                    $pos_category_over_picture = true;
                    $show_category = true;
                    $pos_category = 'picture';
                    break;
                case 'title':
    ...
    $unauthorized_cat_link_text = trim($params->get('unauthorized_cat_link', ''));
    ...
    if ($show_category) {
        ...
        if ($cat_read_more_style == 'bootstrap' && $bootstrap_version > 0 && $link_category) {
            ...
            if ($pos_category == 'picture' && $cat_read_more_classes == '') {
    ...
    $use_leading = $params->get('use_leading', 0);
    $leading_items_count = intval($params->get('count_leading', ''));
    $remove_head = false;
    $remove_text = false;
    $remove_details = false;
    if ($use_leading && $leading_items_count > 0) {
        $remove_head = $params->get('no_head', true);
        $remove_text = $params->get('no_text', false);
        $remove_details = $params->get('no_details', false);
    }
    ...
    if (in_array($head_type, $image_types) || substr($head_type, 0, strlen('jfield:media')) === 'jfield:media' || substr($head_type, 0, strlen('jfieldusers:media')) === 'jfieldusers:media' ||
        substr($head_type, 0, strlen('k2field:image')) === 'k2field:image') {
        $show_image = true;
    } else if ($head_type == 'calendar' || substr($head_type, 0, strlen('jfield:calendar')) === 'jfield:calendar' || substr($head_type, 0, strlen('k2field:date')) === 'k2field:date') {
        $show_calendar = true;
    } else if (substr($head_type, 0, strlen('jfield:sywicon')) === 'jfield:sywicon') {
        $show_icon = true;
    } else if (($video_type = LNEHelper::isHeadVideo($head_type)) !== false) {
        $show_video = true;
    }
    ...
    if ($show_icon) {
        ...
        $shadow_width_icon = $params->get('sh_w_icon', 0);

        $hover_effect_icon = $params->get('hover_effect_icon', 'none');
        if ($hover_effect_icon != 'none') {
            $hover_effect_icon = 'hvr-' . $hover_effect_icon;
            // SYWStylesheets::load2DTransitions();
            $transition_method = SYWStylesheets::getTransitionMethod($hover_effect_icon);
            SYWStylesheets::$transition_method();
        }
    }
    ...
    if ($animation) {
        ...
        if (!empty($pagination)) {
            ...
            if ($pagination_position_type == 'title') {
                $module->showtitle = false;

                $module_title = trim($params->get('module_title', '<{headertag} class="{headerclass}">{title}</{headertag}>{pagination}'));

                $patterns = array();
                $patterns[0] = '/{headertag}/';
                $patterns[1] = '/{headerclass}/';
                $patterns[2] = '/{title}/';
                $patterns[3] = '/{pagination}/';

                $replacements = array();
                $replacements[0] = htmlspecialchars($params->get('header_tag', 'h3'));
                $replacements[1] = trim($params->get('header_class', ''));

                $words = explode(" ", $module->title);
                $first_word = $words[0];
                unset($words[0]);

                $replacements[2] = '<span>' . $first_word . '</span>&nbsp;' . implode(" ", $words);
                $replacements[3] = '*pagination*';

                $module_title = preg_replace($patterns, $replacements, $module_title);
                $module_title_array = explode('*', $module_title);
            }
        }
        ...
        // keep to prevent overrides to crash and backward compatibility
        $prev_next = true;
    ...
    if ($params->get('link_to', 'item') == 'item' && $params->get('link_target', 'default') == 'inline') {
    ...
    // TODO remove?
    // handle high resolution images and lazy loading
