...
class HeadselectField extends GroupedlistField
{
	...
	protected function getGroups()
	{
		...
		$group_name = Text::_('PLG_CONTENT_ARTICLEDETAILSPROFILES_VALUE_IMAGEGROUP');
		$groups[$group_name] = array();

		//$options[] = HTMLHelper::_('select.optgroup', Text::_('PLG_CONTENT_ARTICLEDETAILSPROFILES_VALUE_IMAGEGROUP'));

		$groups[$group_name][] = HTMLHelper::_('select.option', 'contact', Text::_('PLG_CONTENT_ARTICLEDETAILSPROFILES_VALUE_LINKEDCONTACT'), 'value', 'text', $disable = false);
		$groups[$group_name][] = HTMLHelper::_('select.option', 'gravatar', Text::_('PLG_CONTENT_ARTICLEDETAILSPROFILES_VALUE_GRAVATAR'), 'value', 'text', $disable = false);
		...
		if ($this->location == 'header') {
			if (isset($customfields['com_content']) && PluginHelper::isEnabled('fields', 'sywicon')) {
				...
				if (!empty($group_options)) {
					...
					$group_name = Text::_('PLG_CONTENT_ARTICLEDETAILSPROFILES_VALUE_ICONGROUP');
					$groups[$group_name] = $group_options;

					//$options[] = HTMLHelper::_('select.optgroup', Text::_('PLG_CONTENT_ARTICLEDETAILSPROFILES_VALUE_ICONGROUP'));
					//$groups[$group_name] = array_merge($groups[$group_name], $group_options);
					//$options[] = HTMLHelper::_('select.optgroup', Text::_('PLG_CONTENT_ARTICLEDETAILSPROFILES_VALUE_ICONGROUP'));
				}
			}
		...
		if ($this->location == 'header') {
			...
			$group_name = Text::_('PLG_CONTENT_ARTICLEDETAILSPROFILES_VALUE_CALENDARGROUP');
			$groups[$group_name] = array();

			//$options[] = HTMLHelper::_('select.optgroup', Text::_('PLG_CONTENT_ARTICLEDETAILSPROFILES_VALUE_CALENDARGROUP'));

			$groups[$group_name][] = HTMLHelper::_('select.option', 'calendar', Text::_('PLG_CONTENT_ARTICLEDETAILSPROFILES_VALUE_CALENDAR'), 'value', 'text', $disable = false);
		...
		if ($fields_exist) {
			...
			foreach ($fieldsPerGroup as $group_id => $groupFields) {
				...
				foreach ($groupFields as $field) {
					$groups[] = HTMLHelper::_('select.option', $prefix.':'.$field->type.':'.$field->id, $groupTitles[$group_id].': '.$field->title, 'value', 'text', $disable = false);
				}
			}
		}
