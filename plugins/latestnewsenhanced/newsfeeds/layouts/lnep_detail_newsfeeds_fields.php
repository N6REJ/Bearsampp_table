...
$icon = empty($icon) ? 'SYWicon-format-list-bulleted': $icon;
$extraclasses = isset($displayData['extraclasses']) ? ' ' . trim($displayData['extraclasses']) : '';

$output = '<ul>';
foreach ($item->feedfields as $key => $field) {
	$output .= '<li>' . $field['name'] . ': ' . implode(', ', explode(',', $field['values'])) . '</li>';
}
$output .= '</ul>';

// using 'echo' to prevent spaces if using direct html tags

echo '<span class="detail detail_newsfeeds_fields' . $extraclasses . '">'; // convention: detail_[information detail type]
...
echo '<span class="detail_data">' . $output . '</span>';
