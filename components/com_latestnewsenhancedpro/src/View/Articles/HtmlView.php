...
/**
 * View class for a blog of articles
 */
...
// 		}
		...
		if (in_array($this->head_type_leading, $image_types) || substr($this->head_type_leading, 0, strlen('jfield:media')) === 'jfield:media' || substr($this->head_type_leading, 0, strlen('jfieldusers:media')) === 'jfieldusers:media'
				|| in_array($this->head_type, $image_types) || substr($this->head_type, 0, strlen('jfield:media')) === 'jfield:media' || substr($this->head_type, 0, strlen('jfieldusers:media')) === 'jfieldusers:media') {
			$show_image = true;
		}

		if ($this->head_type_leading == 'calendar' || substr($this->head_type_leading, 0, strlen('jfield:calendar')) === 'jfield:calendar'
				|| $this->head_type == 'calendar' || substr($this->head_type, 0, strlen('jfield:calendar')) === 'jfield:calendar') {
			$show_calendar = true;
		}

		if (substr($this->head_type_leading, 0, strlen('jfield:sywicon')) === 'jfield:sywicon' || substr($this->head_type, 0, strlen('jfield:sywicon')) === 'jfield:sywicon') {
			$show_icon = true;
		}

		if (Helper::isHeadVideo($this->head_type_leading) !== false || Helper::isHeadVideo($this->head_type) !== false) {
		    $show_video = true;
		}
		...
		$this->categories_unselectable = $this->params->get('cat_index_unselect', array());
		$this->show_category_hierarchy = $this->params->get('show_cat_hierarchy', 0);

		$this->show_tag_picture = $this->params->get('show_tag_pic', 0);
		$this->tag_default_picture = $this->params->get('tag_d_pic', '');
		$this->tags_unselectable = $this->params->get('tag_index_unselect', array());
		$this->show_tag_hierarchy = $this->params->get('show_tag_hierarchy', 0);
				...
				switch ($filter['formfilter']) {
					case 'a' :
						if (!in_array('top_authors', $this->pagination_top_index)) {
							...
							$filters['top_authors'] = array('style' => $filter['ff_style'], 'label' => $filter['ff_label'], 'classes' => '');
							if (isset($filter['ff_classes'])) {
							    $filters['top_authors']['classes'] = $filter['ff_classes'];
							}
						}
						break;
					case 'i' :
					    if (!in_array('top_alias', $this->pagination_top_index)) {
					        ...
					        $filters['top_alias'] = array('style' => $filter['ff_style'], 'label' => $filter['ff_label'], 'classes' => '');
					        if (isset($filter['ff_classes'])) {
					            $filters['top_alias']['classes'] = $filter['ff_classes'];
					        }
					    }
					    break;
					case 't' :
						if (!in_array('top_tags', $this->pagination_top_index)) {
							...
							$filters['top_tags'] = array('style' => $filter['ff_style'], 'label' => $filter['ff_label'], 'classes' => '');
							if (isset($filter['ff_classes'])) {
							    $filters['top_tags']['classes'] = $filter['ff_classes'];
							}
						}
						break;
					case 'c' :
						if (!in_array('top_categories', $this->pagination_top_index)) {
							...
							$filters['top_categories'] = array('style' => $filter['ff_style'], 'label' => $filter['ff_label'], 'classes' => '');
							if (isset($filter['ff_classes'])) {
							    $filters['top_categories']['classes'] = $filter['ff_classes'];
							}
						}
						break;
					case 'p' :
						if (!in_array('top_periods', $this->pagination_top_index)) {
							...
							$filters['top_periods'] = array('style' => $filter['ff_style'], 'label' => $filter['ff_label'], 'classes' => '');
							if (isset($filter['ff_classes'])) {
							    $filters['top_periods']['classes'] = $filter['ff_classes'];
							}
						}
						break;
					default : // custom field - value jfield:list:70 where 70 is the id of the field
						...
						if (!in_array('top_jfield_' . $field_id, $this->pagination_top_index)) {
							...
							$filters['top_jfield_' . $field_id] = array('style' => $filter['ff_style'], 'label' => $filter['ff_label'], 'classes' => '');
							if (isset($filter['ff_classes'])) {
							    $filters['top_jfield_' . $field_id]['classes'] = $filter['ff_classes'];
							}
						}
				}
			}
				...
				switch ($filter['formfilter']) {
					case 'a' :
						if (!in_array('bot_authors', $this->pagination_bot_index)) {
							...
							$filters['bot_authors'] = array('style' => $filter['ff_style'], 'label' => $filter['ff_label'], 'classes' => '');
							if (isset($filter['ff_classes'])) {
							    $filters['bot_authors']['classes'] = $filter['ff_classes'];
							}
						}
						break;
					case 'i' :
					    if (!in_array('bot_alias', $this->pagination_bot_index)) {
					        ...
					        $filters['bot_alias'] = array('style' => $filter['ff_style'], 'label' => $filter['ff_label'], 'classes' => '');
					        if (isset($filter['ff_classes'])) {
					            $filters['bot_alias']['classes'] = $filter['ff_classes'];
					        }
					    }
					    break;
					case 't' :
						if (!in_array('bot_tags', $this->pagination_bot_index)) {
							...
							$filters['bot_tags'] = array('style' => $filter['ff_style'], 'label' => $filter['ff_label'], 'classes' => '');
							if (isset($filter['ff_classes'])) {
							    $filters['bot_tags']['classes'] = $filter['ff_classes'];
							}
						}
						break;
					case 'c' :
						if (!in_array('bot_categories', $this->pagination_bot_index)) {
							...
							$filters['bot_categories'] = array('style' => $filter['ff_style'], 'label' => $filter['ff_label'], 'classes' => '');
							if (isset($filter['ff_classes'])) {
							    $filters['bot_categories']['classes'] = $filter['ff_classes'];
							}
						}
						break;
					case 'p' :
						if (!in_array('bot_periods', $this->pagination_bot_index)) {
							...
							$filters['bot_periods'] = array('style' => $filter['ff_style'], 'label' => $filter['ff_label'], 'classes' => '');
							if (isset($filter['ff_classes'])) {
							    $filters['bot_periods']['classes'] = $filter['ff_classes'];
							}
						}
						break;
					default : // custom field - value jfield:list:70 where 70 is the id of the field
						...
						if (!in_array('bot_jfield_' . $field_id, $this->pagination_bot_index)) {
							...
							$filters['bot_jfield_' . $field_id] = array('style' => $filter['ff_style'], 'label' => $filter['ff_label'], 'classes' => '');
							if (isset($filter['ff_classes'])) {
							    $filters['bot_jfield_' . $field_id]['classes'] = $filter['ff_classes'];
							}
						}
				}
			}
		...
		// get the custom field options for all custom field filters
		$customfield_filters_options = $this->get('CustomFieldsFilterOptions');  
		...
		foreach ($filters as $key => $filter) {
			switch ($key) {
				...
				case 'bot_categories':
					...
					$layout = new FileLayout('pagination.lnepcategoryindex');
					if ($layout_suffix) {
					...
					$data['classes'] = $filter['classes'];
					$data['unselectable'] = $this->categories_unselectable;
					$data['show_hierarchy'] = $this->show_category_hierarchy;
					...
					if ($filter_style === 'list') {
					    if (!$this->params->get('show_cat_hierarchy', 0)) {
					        $filter_style = 'listing flat';
					    } else {
					        $filter_style = 'listing tree';
					    }
					}
					
					$this->pagination_class[$key] = 'category_directory index_filter ' . $filter_style;
					break;

				case 'top_tags': 
				case 'bot_tags':

					$layout = new FileLayout('pagination.lneptagindex');
					if ($layout_suffix) {
						$layout->setSuffixes(array($layout_suffix));
					}
					...
					$data['classes'] = $filter['classes'];
					$data['unselectable'] = $this->tags_unselectable;
					$data['show_hierarchy'] = $this->show_tag_hierarchy;
					$data['bootstrap_version'] = $this->bootstrap_version;
					$data['load_bootstrap'] = $this->load_bootstrap;
					...
					$this->pagination_class[$key] = 'tag_directory index_filter ' . $filter_style;
					break;

				case 'top_authors': 
				case 'bot_authors':

					$layout = new FileLayout('pagination.lnepauthorindex');
					if ($layout_suffix) {
						$layout->setSuffixes(array($layout_suffix));
					}
					...
					$this->pagination_layouts[$key] = $layout;
					$this->pagination_data[$key] = $data;
					$this->pagination_class[$key] = 'author_directory index_filter ' . (($filter['style'] == 'selection') ? 'selection' : 'listing flat');
					break;
					
				case 'top_alias': 
				case 'bot_alias':
				    
				    $layout = new FileLayout('pagination.lnepaliasindex');
				    if ($layout_suffix) {
				        $layout->setSuffixes(array($layout_suffix));
				    }
				    ...
				    $this->pagination_layouts[$key] = $layout;
				    $this->pagination_data[$key] = $data;
				    $this->pagination_class[$key] = 'alias_directory index_filter ' . (($filter['style'] == 'selection') ? 'selection' : 'listing flat');
				    break;
				...
				case 'bot_periods':
					...
					$this->pagination_layouts[$key] = $layout;
					$this->pagination_data[$key] = $data;
					$this->pagination_class[$key] = 'period_directory index_filter ' . (($filter['style'] == 'selection') ? 'selection' : 'listing flat');
				...
				default: // custom fields
					...
					if (isset($customfield_filters_options[$field_id])) {

					    $field_name = $customfield_filters_options[$field_id]['name'];
					    $options_list = $customfield_filters_options[$field_id]['options'];
    					$multiple = false;

						// if the index is also a pre-selected field, only show the values that have been pre-selected

    					if (isset($customfield_filters_arrays[$field_id])) {

						    foreach ($options_list as $prop => $option) {
						        if ($customfield_filters_arrays[$field_id]['inex']) {
						            if (!in_array($option->value, $customfield_filters_arrays[$field_id]['values'])) {
    					...
    					$this->pagination_layouts[$key] = $layout;
    					$this->pagination_data[$key] = $data;
    					$this->pagination_class[$key] = 'field_directory index_filter ' . (($filter['style'] == 'selection') ? 'selection' : 'listing flat');
					}
			}
		}
...
// 		Helper::loadUserStylesheet('blog');
		...
		if ($this->load_more) { // grid view only
		    $ajax_path = Route::_('index.php?option=com_latestnewsenhancedpro&view=articles&format=raw&limit='.$this->params->get('more_count', 3).$this->category_request.$this->tag_request.$this->author_request.$this->alias_request.$this->period_request.$this->fields_request.$this->search_request.$this->search_options_request.$this->order_request.$this->dir_request, false);
		...
		if ($show_icon && $this->params->get('hover_effect_icon', 'none') != 'none') {
			//SYWStylesheets::load2DTransitions();
			$transition_method = SYWStylesheets::getTransitionMethod('hvr-' . $this->params->get('hover_effect_icon'));
			SYWStylesheets::$transition_method();
		}
