...
if ($item->date) {
	...
	if ($item->nbr_years > 0) {
		...
		if ($item->nbr_days == 1) {
		    if ($date_in_future) {
				echo Text::_('COM_LATESTNEWSENHANCEDPRO_INADAY');
			} else {
				echo Text::_('COM_LATESTNEWSENHANCEDPRO_DAYAGO');
			}
		} else {
		    if ($date_in_future) {
		...
		if ($item->nbr_hours == 1) {
		    if ($date_in_future) {
				echo Text::_('COM_LATESTNEWSENHANCEDPRO_INANHOUR');
			} else {
				echo Text::_('COM_LATESTNEWSENHANCEDPRO_HOURAGO');
			}
		} else {
		    if ($date_in_future) {
				echo Text::sprintf('COM_LATESTNEWSENHANCEDPRO_INHOURS', $item->nbr_hours);
			} else {
				echo Text::sprintf('COM_LATESTNEWSENHANCEDPRO_HOURSAGO', $item->nbr_hours);
			}
		}
	} else {
