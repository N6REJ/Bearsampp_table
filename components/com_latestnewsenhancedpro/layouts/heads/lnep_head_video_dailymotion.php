...
//        $(this).has("iframe").length ? $(this).off("click") : $(this).append('<iframe id="{$prefix}_{$item->id}_video" width="$width" height="$height" src="//www.dailymotion.com/embed/video/$videoID?autoplay=1" allow="autoplay" frameborder="0" allowfullscreen></iframe>');
//    });
//});
...
if (preg_match('!^.+dailymotion\.com/(video|hub)/([^_]+)[^#]*(#video=([^_&]+))?|(dai\.ly/([^_]+))!', $item->video[0], $match)) {
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                pb_pa.addEventListener('click', function() {
                    if (document.getElementById('#{$prefix}_{$item->id}_video')) {
                        ...
                        vf.setAttribute('src', '//www.dailymotion.com/embed/video/$videoID?autoplay=1&mute=1');
                        vf.setAttribute('allow', 'autoplay');
...
JS;
        ...
        echo '<iframe id="' . $prefix . '_' . $item->id . '_video" width="' . $width . '" height="' . $height . '" src="//www.dailymotion.com/embed/video/' . $videoID . '" frameborder="0" allowfullscreen></iframe>';
    }
} else {
    if ($show_errors) {
