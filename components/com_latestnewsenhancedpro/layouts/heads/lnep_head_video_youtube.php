...
//        $(this).has("iframe").length ? $(this).off("click") : $(this).append('<iframe id="{$prefix}_{$item->id}_video" width="$width" height="$height" src="https://www.youtube.com/embed/$match[1]?autoplay=1" frameborder="0" allowfullscreen></iframe>');
//    });
//});
...
if (preg_match('%(?:youtube(?:-nocookie)?\.com/(?:[^/]+/.+/|(?:v|e(?:mbed)?)/|.*[?&]v=)|youtu\.be/)([^"&?/ ]{11})%i', $item->video[0], $match)) {

    $poster_src = '';
    if ($item->video_poster) {
        if ($item->video_poster != 'default') {
            $poster_src = $item->video_poster;
        } else {
            $poster_src = '//img.youtube.com/vi/' . $match[1] . '/hqdefault.jpg';
            // hqdefault or 0 480x360 with bars,
            // mqdefault 320x180 (no bars!!!),
            // maxresdefault 1280x720 (no bars!!!)
        }
    }

    if ($poster_src) {

        //echo '<img id="' . $prefix.$item->id . '_button" class="playbutton" src="' . $playButton . '">';
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                pb_pa.addEventListener('click', function() {
                    if (document.getElementById('#{$prefix}_{$item->id}_video')) {
                        ...
                        vf.setAttribute('src', '//www.youtube.com/embed/$match[1]?autoplay=1&mute=1');
...
JS;
        ...
        echo '<iframe id="' . $prefix . '_' . $item->id . '_video" width="' . $width . '" height="' . $height . '" src="//www.youtube.com/embed/' . $match[1] . '" frameborder="0" allowfullscreen></iframe>';
    }
